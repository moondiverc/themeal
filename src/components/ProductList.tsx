    ...
    <main>
      <div className="flex flex-wrap gap-10 m-4 items-start justify-center">
        {items.map((item) => (
          <div
            key={item.idMeal}
            className="bg-white p-4 flex flex-col rounded-lg shadow-md hover:shadow-lg transition-shadow w-[250px]"
          >
            <Link to={`/products/${item.idMeal}`}>
              <div
                className="overflow-hidden w-full h-[200px] bg-center bg-cover rounded-lg"
            ...
            <div className="flex justify-between items-start mt-2">
              <div className="flex-1 min-w-0 mr-2 mt-2">
                <Link
                  ...
                  className="font-bold text-gray-800 hover:text-amber-500 block"
                >
